...
The personalized parameters of individual patients can be obtained by collectively fitting the available experimental data. Here we adapted stochastic ranking evolutionary strategy (SRES) \cite{sres}, a standard global optimization method, to explore the high-dimentional parameter space. %Recent studies \cite{leder14,sorger12} showed that cancer therapy might change the patient state and lead to different therapeutic responses. Thus, we propose an adaptive control based framework. The underlying assumption is that
%
