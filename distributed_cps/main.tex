...
\title{Analyzing Nonlinear Distributed CPS using PALS and SMT
(TO BE WRITTEN LATER)
...
%
But its application to distributed hybrid systems was often quite limited,
because of immediate correlation between physical entities,
...
%(i.e., a hyper-period of the fast components).
...
\begin{inparaenum}[(i)]
	...
	any two components is within %the interval 
	$[\mu_{\min}, \mu_{\max}]$.
\end{inparaenum}
...
provided that 
$T \geq \mu_{\max} + 2 \epsilon + \max(2\epsilon - \mu_{\min}, \alpha_{\max})$ holds.
%
...
(adapted from \cite{chaki2013compositional}).
..
