...
Suppose that  a distributed CPS is specified 
...
In principle, we can model check the system
by simulating its all possible behavior of the system,
where the parameters of interest are precisely given.
For example, we can easily extend the execution framework
in \cite{ftscs-journal} for this purpose.

However, in general,
there are often an infinite number of such parameters for the system,
so that a model checking verification is typically performed
only for specific parameters of the system.
For example, 
an exact local clock $c_M$ of each controller in the system
can vary, and in the thermostat example in Section~\ref{sec:thermo-example},
the main controller can give an infinite number of $t_{\max}$ and $t_{\min}$.
Therefore, model checking verifications based on explicitly simulating the system
is not satisfactory in general,
because for other parameters, the system may not satisfy the requirements.

This section explains how we can verify the system 
for such an infinite number of parameters.
Section~\ref{sec:formula} shows that a complete behavior of the system can be
expressed a logic formula up to a given bound.
The safety requirements of the system is also expressed as a certain logic formula.
This means that by checking the satisfiability of such formulas, 
we can verify that the system requirements are satisfied up to a certain bound.
Section~\ref{sec:component-dReal}
explains how the satisfaction problems of such formulas generated by our framework
can be automatically checked up to any precision $\delta > 0$
using the \texttt{dReal} SMT solver.
...
Section~\ref{sec:thermo-smt-example} then illustrates our approach
using the thermostat example in Section~\ref{sec:thermo-example}.
