...
For a fixed tank $k$ and a subinterval $I' = [L_{\min} - \eta', L_{\max} + \eta']\subseteq I$
with $\eta' < \eta$,
we show that $x_{k} \in I'$ is an inductive condition
(that is, $(x_k(0) \in I' \wedge \phi_{\mathcal{E} \restriction_{\Pi} E_\mathcal{E}}^{T,0})
\rightarrow x_k(T) \in I'$),
provided  that $x_{k-1} \in I$ always holds. % for its input tank  $k-1$.
We then show that $x_{k} \in I$
if $x_{k} \in I'$ at the beginning of each round.

In the analysis, we use the parameters 
$a = 0.5$, $g = 9.8$, $A = 2$, $q = 4$, $L_{\min} = 8$, $L_{\max} = 10$,
$\eta = 3$, $\eta' = 2$, and $T = 1\,\mathrm{s}$.
%
For the maximal clock skew is $\epsilon = 3\,\mathrm{ms}$,
the sampling time is $t_I = 20\,\mathrm{ms}$,
and the response time $t_R = 10\,\mathrm{ms}$,
...
%However, if ? = 0.02, then the inductive condition is violated by the trajectories in Fig. 8, which were generated by dReal, using precision ? = 0.001.7 Figure 8 shows that the temperature of x2 rises for extra 0.02 + ? time units (where the round begins at time ? = 0.02), so that the value of x2 at time 0.2 (i.e., ? + 0.2 ? ?) can be greater than 22.
...
A thermostat controller  \cite{henzinger2000theory} is installed in each room.
...
\[
\begin{aligned}
\dot{x}_i &= K_i \big(h_i - ((1- k_{i-1} - k_{i+1}) x_0 + k_{i-1} x_{i-1} + k_{i+1} x_{i+1})\big)
\\ 
&\mbox{if}\;\; m_i = m_\texttt{on},
\\
\dot{x}_i &= - K_i \big((1- k_{i-1} - k_{i+1}) x_0 + k_{i-1} x_{i-1} + k_{i+1} x_{i+1}\big)
\\
& \mbox{if}\;\; m_i = m_\texttt{off},
\end{aligned}
\]
...
\path (2.5,-1.5) node[draw] (m2){ $H_{k+1}$};
%right wall
