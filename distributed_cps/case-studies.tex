...
This section shows %four 
case studies %and experimental results 
for SMT-based analysis of 
...
$I = [L_m - \eta, L_M + \eta]$.
%
We have verified the safety property for an \emph{any number} of connected water tanks 
...
(i.e., $(x_k(0) \in I' \wedge \phi_{\mathcal{E} \restriction_{\Pi} E_\mathcal{E}}^{T,0}) \rightarrow (x_k(T) \in I') \wedge (\forall t \in [0,T].\; x_{k}(t) \in I)$).}
%
...
Similarly, we have verified the safety property $\forall t.\; x_i \in I$
...
%by checking the unsatisfiability of the negated formulas,
 using \textsf{dReal} with precision $\delta = 0.001$ (the analysis took $2.6$ seconds).%
 \footnote{In the analysis, we use the parameters $K = 0.015$, 
$h = 100$,
$c = 0.01$,
$T_M = 21$, 
$T_m = 19$,
...
are summarized  in Fig.~\ref{table:bounded}.%
\footnote{We consider both \texttt{unsat} (verified) and \texttt{sat}  (counterexample found) cases
by using different safety properties.}
%
The performance of the new encoding 
is dramatically better than one of the non-modular encoding.
For example, 
the SMT analysis using the non-modular encoding
for the three interconnected thermostats
