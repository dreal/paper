...
A state of a physical environment %of a machine $M$ 
is given by 
...
%(a trajectory of duration $T$ is a function $\tau : [0,T] \rightarrow \mathbb{R}$).
...
Several physical environments can be physically correlated to each other,
and one %local 
environment may immediately affect  another environment.
Such physical correlations are %naturally 
expressed as
...
For instance, if %physical 
parameter $x_1$ of $E_{M_1}$
must be equal to parameter $x_2$ of %another environment 
$E_{M_2}$,
then the time-invariant constraint is expressed as %the formula
...
\draw[<-,densely dotted,semithick] (0.72,0.39) -- (0.72,0.84) node[above] {$c_M(i) + t_I$};
\draw[<-,densely dotted,semithick] (1.55,0.34) -- (1.55,0.84) node[above] {$c_M(i) + t_R$};
\end{scope}

\filldraw (0,0.14) circle (0.5pt) node[above,xshift=-1.2ex] {$\vec{v}$};
\draw (-0.1,0.11) .. controls (0.5,0.3) and (0.9,0.5) .. (1.55,0.3);
\filldraw (0.26,0.23) circle (0.5pt);
\filldraw (0.72,0.35) circle (0.5pt) node[below] {$\vec{v}_I$};
\filldraw (1.55,0.3) circle (0.5pt) node[below] {$\vec{v}_R$};
\draw (1.55,0.3) .. controls (2,0.1) and (2.5,0.3) .. (3.2,0.3);
\filldraw (2.6,0.26) circle (0.5pt) node[below,xshift=1.2ex] {$\vec{v'}$};
...
before time $i T - \epsilon$.
After some sampling time $t_I$ has elapsed,
...
machine state $s$.
%
After the execution finished at time  $c_M(i) + t_R$, 
its machine state changes to the next one $s'$,
where $t_R$ denotes the execution time.
...
Deciding $\pi$ is one of system design choices. 
