...
$\frac{\mathrm{d}\vec{x}}{\mathrm{d}t}= F(\vec{x},t)$, then the formula %$\phi_{E_M}(\vec{a},u,\vec{v},\vec{f})$ 
...
%A formula $\phi_{E_M}(\vec{a},u,\vec{v} \shortmid \vec{x})$ specifies 
%the behavior of $\vec{x}$
%for the time interval $[0,u]$.
%We define the ``time-shifted'' formula $\phi_{E_M}^{+u_0}$ 
%to specify the behavior of $\vec{x}$ for the interval $[u_0, u_0 + u]$.
%It is  obtained from $\phi_{E_M}$
%by replacing each occurrence of a term $\vec{x}(t)$
%by $\vec{x}(t+u_0)$.
%For example, 
%(\ref{eq:uq})
%is transformed into
%$(\forall t)\; (0 \leq t \leq u) \rightarrow (\vec{x}(t+u_0) = \vec{v}_M 
%+ \int_0^{t}  F(\vec{x},t)\,\mathrm{d}t$.
