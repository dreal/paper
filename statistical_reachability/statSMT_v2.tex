...
large optimization problems, and become infeasible for systems involving nonlinear dynamics
...
to solve reachability analysis in sound way, i.e., we always decide correctly if the system 
actually reaches the unsafe region (in the opposite case we may generate false positives, but
this can be controlled by a precision parameter $\delta>0$).
...
hybrid systems with probabilistic systems parameters by applying it to a representative
