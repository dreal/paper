...
We can now define the following formula that checks whether a {\em goal} region is reachable after 
...
We have developed an open-source tool dReach using OCaml to perform $\delta$-complete reachability analysis for hybrid systems (\url{http://dreal.cs.cmu.edu/dreach.html}). dReach is built upon our SMT solver dReal \citep{dreal} that implements a $\delta$-complete decision procedure.
