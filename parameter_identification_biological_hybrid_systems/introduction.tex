...
behavior in various `discrete' states. For example, the five stages of the cell cycle are driven 
...
will evolve according to the flow in the mode. Jump conditions control the switching to another mode,
...
We show the applicability of our method by carrying out two case studies characterized by highly
nonlinear hybrid models. The first one involves a model aimed at studying hormone therapy for 
prostate cancer \cite{ideta08}.
We show that our method is able to perform model selection by ruling out model candidates that 
do not fit experimental observations. We also used our method to optimize personalized treatment 
schemes for individual patients to achieve maximum therapeutic efficacy.
In the second case study we analyzed a cardiac cell model \cite{orovio08}. We identified parameter 
ranges for which a cardiac cell may lose excitability.
