...
Decision solvers for logic formulas over the real numbers play a crucial role in the formal verification of safety-critical embedded systems. To guarantee correctness, decision solvers should provide, besides ``{\sf sat}/{\sf unsat}" answers, certificates for the answers that can be easily validated. For {\sf sat} answers, a solution (value assignments for all variables) usually suffices. For {\sf unsat} answers, we need mathematical {\em proofs of unsatisfiability}, which can be more tricky. This is especially a problem in the framework of $\delta$-complete decision procedures over the reals~\cite{}, because of its use of numerical procedures that necessarily contain numerical approximations and floating point arithmetic. We address this problem in this paper. 

Our solver dReal~\cite{} uses Interval Constraint Propagation (ICP)~\cite{handbookICP} to find solutions of constraints over the reals using a ``branch-and-prune'' method. Theoretically, correctness of the ``no solution" answers can be guaranteed as long as the pruning operators are {\em conservative}, which means it never discards actual solutions of the system~\cite{handbookICP,DBLP:conf/cade/GaoAC12}. In practice, however, constraint solvers are themselves complex software and the theoretical guarantees are easily violated in actual implementation. For instance, the following is an actual bug we experienced in building our SMT solver dReal~\cite{bug}. 
\begin{quote}
...
\end{quote}
This shows the importance of validating the results, through proofs that can ensure the correctness of ``no solution" results and reject the wrong answers. It is worth noting that such problems are not obvious when constraint solving actually obtains solutions, whose correctness can be checked by plugging them back in the original constraints. This is why we emphasize on proofs of {\em unsatisfiability}, for which we do not have direct witnesses to check. 
