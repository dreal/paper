...
Solving constraints over the real numbers plays an important role in the formal verification of embedded software~\cite{DBLP:conf/cade/GaoAC12}. Using standard verification techniques, we can formulate safety properties of embedded software as constraint systems, whose solutions witness software bugs. In most cases, we conclude that a piece of software is safe, if its corresponding encoding as a constraint system does {\em not} have a solution. In short, ``no solution" means ``safety", and constraint solvers are used for ensuring the {\em absence} of solutions. Thus, a new requirement is posed on constraint solvers. Simply returning ``no solution" is not enough, and valid ``proofs of unsatisfiability" are needed, so that the answers can be trusted and used for formal verification.

Interval Constraint Propagation (ICP)~\cite{handbookICP} finds solutions of constraints over the reals using a ``branch-and-prune'' method. Theoretically, correctness of the ``no solution" answers can be guaranteed as long as the pruning operators are {\em conservative}, which means it never discards actual solutions of the system~\cite{handbookICP,DBLP:conf/cade/GaoAC12}. However, in practice, constraint solvers are themselves complex software that involves sophisticated numerical heuristics and error-prone floating point arithmetic. Theoretical guarantees are easily violated in actual implementation. For instance, the following is an actual bug we experienced in building our SMT solver dReal~\cite{dreal,bug}. 
\begin{quote}
