...
\begin{align*}
  ...
  % variable-decl
  \mathit{macro\_decl} \ := \ &  \mathtt{\#define} \ \mathit{var} \ (\mathit{expr} \, | \, \mathit{formula})\\
  % variable-decl
  \mathit{variable\_decl} \ := \ &  \mathtt{[}
  \mathit{l}
  \mathtt{,}
  \ \mathit{u}
  \mathtt{]}
  ...
  \mathtt{;}\\
  % variable-decl
  \mathit{mode\_def} \ := \ & \ \mathtt{\{}
  \mathtt{mode} \
  \mathit{id}\mathtt{;} \quad
  \mathtt{invt}:(\mathit{formula} \mathtt{;})^+ \ \ \
  \mathtt{flow}:\mathit{ode}^+ \ \ \ \mathtt{jump}:\mathit{jump}^+ \mathtt{\}}\\
  \mathit{ode} \ := \ & \mathtt{d/dt[}\mathit{x}\mathtt{]=}\mathit{exp}\\
  \mathit{jump} \ := \ & \mathit{formula} \ \texttt{==>} \ \texttt{@}\mathit{n} \ \mathit{formula}\\
  \mathit{initial\_cond} \ := \ & \texttt{@}\mathit{mode\_id} \ \mathit{formula}\mathtt{;}\\
  \mathit{goal}              \ := \ & \texttt{@}\mathit{mode\_id} \ \mathit{formula}\mathtt{;}
\end{align*}
